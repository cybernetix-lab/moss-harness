...
```bash
# 检查配置文件是否存在
ls -la agents/${ECC_AGENT}.yaml

# 验证配置格式
yq eval agents/${ECC_AGENT}.yaml > /dev/null && echo "配置有效"

# 查看详细错误
./scripts/agent-start.sh ${ECC_AGENT} --verbose
```
...
```bash
# 检查最近的评估结果
./scripts/agent-eval.sh status ${ECC_AGENT}

# 查看进化历史
./scripts/agent-evolve.sh status ${ECC_AGENT}

# 回滚到上一版本
./scripts/agent-evolve.sh rollback ${ECC_AGENT}
```
...
```bash
# 检查当前 Agent 的工具权限
cat agents/${ECC_AGENT}.yaml | yq '.tools'
