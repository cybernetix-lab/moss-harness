...
# 接收消息
./core/mailbox.sh receive --agent reviewer --unread

# 标记已读
./core/mailbox.sh mark-read --agent reviewer --msg msg-001

# 查询线程状态
./core/mailbox.sh status --thread task-001

# 列出消息
./core/mailbox.sh list --agent planner --box inbox

# 归档消息
./core/mailbox.sh archive --agent planner --older-than 7

# 清理回收站
./core/mailbox.sh cleanup --agent planner
```

### 2. 消息处理器 (core/message-handler.sh)

处理消息的解析、验证、路由和转换。

```bash
# 解析消息
./core/message-handler.sh parse runtime/mailbox/planner/inbox/msg-001.json

# 验证消息
./core/message-handler.sh validate runtime/mailbox/planner/inbox/msg-001.json

# 路由消息
./core/message-handler.sh route runtime/mailbox/planner/outbox/msg-002.json

# 转换格式
./core/message-handler.sh transform msg-001.json yaml
./core/message-handler.sh transform msg-001.json summary

# 创建回复
./core/message-handler.sh reply msg-001.json '{"status":"approved"}'

# 转发消息
./core/message-handler.sh forward msg-001.json executor

# 广播消息
./core/message-handler.sh broadcast SYSTEM_NOTIFICATION '{"message":"系统维护"}'
```

### 3. 投递守护进程 (core/delivery-daemon.sh)
...
# 查看状态
./core/delivery-daemon.sh status

# 运行一次投递
./core/delivery-daemon.sh once

# 停止守护进程
./core/delivery-daemon.sh stop

# 重启守护进程
./core/delivery-daemon.sh restart
```
...
# 1. 创建所有 Agent 邮箱
./core/mailbox.sh create --agent planner
./core/mailbox.sh create --agent reviewer
./core/mailbox.sh create --agent executor
./core/mailbox.sh create --agent evaluator

# 2. 启动投递守护进程
./core/delivery-daemon.sh start

# 3. 创建任务线程
./core/mailbox.sh create-thread --thread task-001
...
# 5. Planner 接收任务
./core/mailbox.sh receive --agent planner --unread
...
```bash
# crontab 中添加
0 0 * * * cd /path/to/project && ./core/mailbox.sh archive --agent planner --older-than 30
0 0 * * * cd /path/to/project && ./core/mailbox.sh cleanup --agent planner
```
...
```bash
# 验证消息格式
./core/message-handler.sh validate msg-file.json

# 查看详细错误
./core/message-handler.sh validate msg-file.json | jq '.errors'
```
...
curl -X POST http://api.example.com/webhook | \
  ./core/mailbox.sh send --from system --to planner --type EXTERNAL_EVENT --body -

# 发送消息到外部系统
./core/mailbox.sh receive --agent evaluator --unread | \
