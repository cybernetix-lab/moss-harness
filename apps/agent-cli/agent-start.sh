...
echo ""
echo "Context files:"
ls -1 "${PROJECT_ROOT}/.runtime/context/" 2>/dev/null | sed 's/^/  - /'
echo ""
            ...
            while true; do
                ...
                case "$cmd" in
                    status)
                        ...
                        ;;
                    context)
                        ls -la "${PROJECT_ROOT}/.runtime/context/" 2>/dev/null || echo "No context files"
                        ;;
                    skills)
...
check_and_compact_context() {
    ...
    if [[ -x "$COMPACTOR" ]]; then
        log_step "Checking context size..."
        "$COMPACTOR" auto "$session_dir" 2>/dev/null || true
    fi
}
...
main() {
    ...
    # 检查上下文大小并自动压缩
    check_and_compact_context "$session_dir"
