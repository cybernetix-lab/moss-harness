...
get_skill_stats() {
    local skill_name="$1"
    local stats_file="${PROJECT_ROOT}/runtime/memory/skill-stats/${skill_name}.json"
...
analyze_skill() {
    ...
    local config_file="${PROJECT_ROOT}/integrations/skills/evolution/config.yaml"
...
extract_patterns() {
    ...
    local sessions_dir="${PROJECT_ROOT}/runtime/memory/sessions"
...
check_skill_similarity() {
    ...
    for skill_file in "${PROJECT_ROOT}"/integrations/skills/*/*/skill.yaml; do
        if [[ -f "$skill_file" ]]; then
...
extract_patterns_from_memory() {
    ...
    local sessions_dir="${PROJECT_ROOT}/runtime/memory/sessions"
    local patterns_dir="${PROJECT_ROOT}/runtime/memory/patterns"
    mkdir -p "$patterns_dir"
...
discover_new_skills() {
    ...
    local proposals_dir="${PROJECT_ROOT}/integrations/skills/evolution/proposals"
...
review_proposal() {
    ...
    if [[ "$action" == "view" || -z "$action" ]]; then
        ...
        local skill_dir="${PROJECT_ROOT}/integrations/skills/${skill_category}/${suggested_name}"
        mkdir -p "$skill_dir"
